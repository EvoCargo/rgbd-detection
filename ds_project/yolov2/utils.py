...
def bbox_ious(boxes1, boxes2):
    """
    Calculate IOU metric between two bounding boxes

    Args:
        boxes1 (torch.Tensor): ground truth bounding box
        boxes2 (torch.Tensor): predicted bounding box
...
def post_processing(logits, image_size, gt_classes, anchors, conf_threshold, nms_threshold):
    """
    Prepare best matches bounding boxes

    Args:
        logits (torch.Tensor): predictions from model
        image_size (tuple): Image width and height
        gt_classes (list): names of classes
        anchors (list): anchor boxes
        conf_threshold (float): threshold for confidence
        nms_threshold (float): threshold for non-maximum suppression

    Returns:
        list: most confident predictions
    """
